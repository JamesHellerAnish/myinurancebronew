...
4. ✅ **Fixed — fonts are self-hosted.** `css/fonts.css` + `assets/fonts/`, generated by
   `site/scripts/fetch-fonts.mjs`. Zero third-party requests on any page.
...
#### ✅ Re-measured after both fixes — 2026-09-10

| | Before | After |
|---|---|---|
| Home LCP (unthrottled localhost) | 1,416 ms | **264 ms** — 5.4× faster |
| CLS | 0 | **0** — unchanged |
| `chief-advisor` | 2,341,112 B PNG | **153.8 KB WebP** — 93.3% smaller |
| Portrait natural size | 1471×1809 | 880×1082 (displayed 404×505) |
| Third-party requests | fonts.googleapis.com + fonts.gstatic.com | **none — zero third-party requests** |

**The portrait** is now `assets/images/chief-advisor.webp`, generated by
`site/scripts/make-portrait.mjs` from the original in `assets/images/originals/`. Width is
derived from the source aspect ratio rather than hard-coded, so a re-shot portrait cannot
silently stretch, and the `<img`> `width`/`height` were updated to 880×1082 so the reserved
box stays truthful and CLS stays at zero. On Lighthouse's Slow 4G that file drops from
**11.7 s to about 0.8 s**.

**The fonts** are self-hosted in `assets/fonts/`, with `css/fonts.css` generated by
`site/scripts/fetch-fonts.mjs`. Two narrowings against what Google serves: variable fonts
(`wght@400..800`, one file per family+subset instead of one per weight) and Latin subsets only
— the default reply also carries Cyrillic, Greek and Vietnamese, which nothing on this site can
render. Inter latin is preloaded. Both the Astro layout and the legacy `index.html` were
switched, so the two halves of the migration stay in step.

Note `inter-latin-ext.woff2` (83 KB) still loads on pages showing prices: **₹ (U+20B9) falls
in the latin-ext range**. That is not waste — it is the currency symbol — but it is worth
knowing before anyone tries to drop the subset.

#### What this means for the gate

The Phase 0 → 1a gate is "site live, CWV green, indexed". Both lab blockers are now fixed:
CLS is a true zero, plan pages render in under 100 ms, the home page LCP is 264 ms, the
render-blocking CSS is 19 KB gzipped and **no request leaves the origin**.

**That is not the same as "CWV green".** These are still unthrottled localhost numbers. The
gate needs a measurement against a deployed URL — or a throttled Lighthouse run, which is not
