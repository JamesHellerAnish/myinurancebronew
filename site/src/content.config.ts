...
/**
 * §1027 — the glossary. `definition` is verbatim policy-wording text with its UIN; `plain`
 * and `whyItMatters` are ours. Records are generated by scripts/build-glossary.mjs, which
 * only writes a term once it has an editorial layer, so every published term has both halves.
 */
const glossary = defineCollection({
  ...
  schema: z.object({
    slug: z.string(),
    term: z.string(),
    /** Verbatim from the wording — never paraphrase this into our own words. */
    definition: z.string(),
    source: z.object({
      insurer: z.string(),
      document: z.string(),
